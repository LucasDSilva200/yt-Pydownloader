...
def convert_to_mp3(yt,name,filepath):
        stream = yt.streams.get_highest_resolution()
        stream.download(os.path.join(tempfile.gettempdir()),filename=f"{name}.mp4")
        video_path = os.path.join(tempfile.gettempdir(), stream.default_filename)
        banner_url = yt.thumbnail_url
        banner_file = requests.get(banner_url).content
        temp_banner = os.path.join(tempfile.gettempdir(), "banner.jpg")
        ...
        with open(temp_banner, "rb") as f:
            image_data = f.read()
        audio.add(APIC(encoding=3, mime='image/png', type=3, desc=u'Cover', data=image_data))
